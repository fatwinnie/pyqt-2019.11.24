...
class CamShow(QMainWindow,Ui_CamShow):
    ...
    def drawAvg(self):
        ...
        for i in range(640):
            r_arr[i,0] = np.sum(img2RGB[260:280 , i , 0] )/ int(self.roi_Height.text())
            g_arr[i,0] = np.sum(img2RGB[260:280 , i , 1] )/ int(self.roi_Height.text())
            b_arr[i,0] = np.sum(img2RGB[260:280 , i , 2] )/ int(self.roi_Height.text())
            Gray_arr[i,0] = np.sum(gray[260:280 , i ])/ int(self.roi_Height.text())

        self.SavePixel_arr = np.zeros((640,2),dtype=np.float16)  # SavePixel_arr 存Gray_arr
        for i in range(640):
            #self.save_arr[i,0] = (x_640 - x_0) / 640 * (i + 1)
            self.SavePixel_arr[i,0] = i
            self.SavePixel_arr[i,1] = Gray_arr[i,0]
    ...
    def drawAvg_after_calculate(self):
        ...
        self.save_arr = np.zeros((640,2),dtype=np.float16)  # save_arr存Gray_arr
        
        
        for i in range(640):
            #self.save_arr[i,0] = (x_640 - x_0) / 640 * (i + 1)
            self.save_arr[i,0] = (x_640 - x_0) / 640 * i
        ...
        self.y_arr = np.zeros((640,1),dtype=np.float16)
        for arr in self.getSave_arr:
            for i in range(640):
                self.y_arr[i,0] = np.add(self.y_arr[i,0], arr[i,1])
            

        self.Gray_arr = np.zeros((640,1),dtype=np.float16)  
        for i in range(640):
    ...
    def saveLog(self):
        ...
        # 以下: 存波長對強度 
        self.y_arr = np.zeros((640,1),dtype=np.float16)  #y_arr = [] #存 getSave_arr相加的值
        for arr in self.getSave_arr:
            for i in range(640):
                self.y_arr[i,0] = np.add(self.y_arr[i,0], arr[i,1])
        ...
        # 以下:存pixcel對強度
        self.add_arr = np.zeros((640,1),dtype=np.float16)
